...
# just for funsies
noise = np.random.normal(0, 1e-2, np.shape(data.y))

fig, ax = plt.subplots(1, 2)

ax[0].scatter(data.t, data.y[0] + noise[0, :])
ax[0].set_title('Position vs. Time (Gaussian Noise, StdDev 1e-2)')
ax[0].set_xlabel('Time [s]')
ax[0].set_ylabel('Position [m]')

ax[1].scatter(data.t, data.y[1] + noise[1, :])
ax[1].set_title('Velocity vs. Time (Gaussian Noise, StdDev 1e-2)')
ax[1].set_xlabel('Time [s]')
ax[1].set_ylabel('Velocity [m/s]')
