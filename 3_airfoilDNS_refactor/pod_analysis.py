...
for k in range(6):
    ...
    im = ax.contourf(
        x,
        y,
        np.transpose(U_ux[:, k].reshape(nx, ny, order="F")),
        levels=MM * v,
        vmin=-MM,
        vmax=MM,
    )
...
for k in range(6):
    ...
    im = ax.contourf(
        x,
        y,
        np.transpose(U_uy[:, k].reshape(nx, ny, order="F")),
        levels=MM * v,
        vmin=-MM,
        vmax=MM,
    )
...
t_star = np.linspace(0, nt / 2 - 1, 6)
# from temporal amplitudes we see that we get two periods out of this bad boy
...
for k in range(6):
    # outlier clipping
    cbar_max = np.append(
        cbar_max, np.quantile(X_approx[0 : nx * ny, int(t_star[k])], 0.99)
    )
    cbar_min = np.append(
        cbar_min, np.quantile(X_approx[0 : nx * ny, int(t_star[k])], 0.01)
    )
...
for k in range(6):
    ax = ax4.flat[k]
    ux_approx = np.transpose(X_approx[0 : nx * ny, int(t_star[k])])
    ux_approx = ux_approx.reshape(nx, ny, order="F")
    im = ax.contourf(
        x,
        y,
        np.transpose(ux_approx),
        levels=np.linspace(cbar_min, cbar_max, 40),
        vmin=cbar_min,
        vmax=cbar_max,
        extend="both",
    )
...
for k in range(6):
    ax = ax5.flat[k]
    uy_approx = np.transpose(X_approx[nx * ny :, int(t_star[k])])
    uy_approx = uy_approx.reshape(nx, ny, order="F")
    im = ax.contourf(
        x,
        y,
        np.transpose(uy_approx),
        levels=np.linspace(cbar_min, cbar_max, 40),
        vmin=cbar_min,
        vmax=cbar_max,
        extend="both",
